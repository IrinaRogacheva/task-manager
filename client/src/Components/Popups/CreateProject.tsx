...
export default function CreateProject(props: any) {
  ...
  return (
  <div style={{display: (view.isCreateProject?'block':'none')}}>
    ...
    <div className='create_project context-menu'>
        <div className='create_project_title-wrapper'>
            <p className='create_project__title'>Создание проекта</p>
            <div className='exit_icon' onClick={()=>{dispatch(setCreateProject(false));setInputValue("");setColor(colors.COLOR18)}}>
        ...
        <div className='create_project_buttons-wrapper'>
            <button onClick={()=>{dispatch(setCreateProject(false));setInputValue("");setColor(colors.COLOR18)}} className='create_project_cancel_button sidebar__text'>Отмена</button>
            <button onClick={()=>{createProject();dispatch(setCreateProject(false))}} disabled={inputValue.length<1 && inputValue.replace(/\s/g, '').length > 0} className='create_project_save_button sidebar__text'>Сохранить</button>
        </div>
    </div>
  </div>
);}